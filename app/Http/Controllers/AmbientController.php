...
class AmbientController extends Controller {
    public function index() {
        $ambiente=[];
        foreach (Ambient::get() as $valor) {
            $valor->personal = json_decode($valor->personal);
            $ambiente[]=$valor;
        }
    ...
    public function show($id) {
        //dd($id);
        $ambient = Ambient::find($id);
        $ambient->personal = json_decode($ambient->personal);
        $usuarios = Usuariosprofit::get();
        return view('ambiente.show', compact('ambient', 'usuarios'));
    }
}
